<?php
	...
	if($_POST){
		if(isset($_POST['id'])){
			$item = $db->executeQuery("select * from visualization_2013 where id = ".$_POST['id']);
		...
		if($_POST["visualization-select"] === 'file'){
			...
			copy($_FILES["visualization-file"]["tmp_name"], dirname(__FILE__) . '/dat2013/tmp/'.$tmpFileName);
		}
	} else {
		...
		<tr>
			...
			<td>
				<div class="designate-right">
					...
					<?php } else { ?>
						<a href="<?php echo BASE_URL.'dat2013/tmp/'.$tmpFileName; ?>" target="_blank"><?php $splitted = explode("_", $tmpFileName, 2); echo $splitted[1]; ?></a>
