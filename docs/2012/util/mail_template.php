...
class MailTemplate {
	...
	static function datasetMail($id, $name, $affiliation, $affiliation_anonymous, $email, $email_anonymous, 
		$dataset_name, $dataset_url, $abstract, $propose, $related_dataset_ids, $related_idea_ids, $related_visualization_ids, $right, $password, $creator){
		...
		$str .= "\n";
		$str .= "http://lod.sfc.keio.ac.jp/challenge2012/show_status.php?id=d".sprintf("%03d", $id)."\n";
	...
	static function ideaMail($id, $name, $affiliation, $affiliation_anonymous, $email, $email_anonymous, 
		$idea_name, $open, $abstract, $related_dataset_ids, $related_application_ids, $related_visualization_ids, $type, $idea_url, $password){
		...
		$str .= "\n";
		$str .= "http://lod.sfc.keio.ac.jp/challenge2012/show_status.php?id=i".sprintf("%03d", $id)."\n";
		$str .= "修正パスワード： ".$password."\n";
		$str .= "\n";
	...
	static function visualizationMail($id, $name, $affiliation, $affiliation_anonymous, $email, $email_anonymous, 
		$visualization_name, $abstract, $related_dataset_ids, $related_idea_ids, $type, $visualization_url, $copyright, $license, $password){
		...
		$str .= "e-mailアドレス	".$email." [".($email_anonymous ? "非公開" : "公開")."]"."\n";
		$str .= "\n";
		$str .= "◆　応募したビジュアライゼーションの情報"."\n";
		$str .= "\n";
		$str .= "ビジュアライゼーションの名称	".$visualization_name."\n";
		$str .= "ビジュアライゼーションの概略説明	 ".$abstract."\n";
		$str .= "関連するデータセット	".$related_dataset_ids."\n";
		$str .= "関連するアイデア	".$related_idea_ids."\n";
		$str .= "投稿したビジュアライゼーション	".($type == 'url' ? $visualization_url : $type)."\n";
		...
		$str .= "作品が公開された後、登録内容の確認・修正は以下のURLから行えます。";
		$str .= "\n";
		$str .= "http://lod.sfc.keio.ac.jp/challenge2012/show_status.php?id=v".sprintf("%03d", $id)."\n";
		$str .= "修正パスワード： ".$password."\n";
		$str .= "\n";
		$str .= "作品の応募後は、「応募者の情報」「応募したビジュアライゼーションの情報のうち\n";
		$str .= "ビジュアライゼーションの名称/ビジュアライゼーションの概略説明/関連するデータセット/関連する\n";
		$str .= "アイデア」の修正しかできません。\n";
 		$str .= "また応募作品の取り消しはできません。（応募規程7,応募規程9)\n";
 		$str .= "本欄以外での修正、取り消しなどの要望がありましたらlod-challenge@sfc.keio.ac.jp\n";
 		$str .= "までご連絡下さい。\n";
	...
	static function applicationMail($id, $name, $affiliation, $affiliation_anonymous, $email, $email_anonymous, 
		$application_name, $application_url, $abstract, $related_dataset_ids, $related_idea_ids, $related_visualization_ids, $right, $password, $license){
		$cr = Copyright::getInstance();
		...
		$str .= "登録内容の確認・修正は以下のURLから行えます。\n";
		$str .= "\n";
		$str .= "http://lod.sfc.keio.ac.jp/challenge2012/show_status.php?id=a".sprintf("%03d", $id)."\n";
		$str .= "修正パスワード： ".$password."\n";
		$str .= "\n";
