<?php
	...
	if($_POST){
		...
		$ret = $fc->notEmpty($_POST["basetechnology-name"]);
		if($ret !== true) $errorMessages["basetechnology-name"] = $ret;
		
		$ret = $fc->notEmpty($_POST["basetechnology-url"]);
		if($ret !== true) $errorMessages["basetechnology-url"] = $ret;
		
		$ret = $fc->regex($_POST["basetechnology-url"], "^https?:\/\/");
		if($ret !== true) $errorMessages["basetechnology-url"] = $ret;
		
		$ret = $fc->checkLength($_POST["basetechnology-abstract"], 100);
		if($ret !== true) $errorMessages["basetechnology-abstract"] = $ret;
		...
		if(empty($errorMessages)){ // データベースなどへの登録処理
			require("check_basetechnology_input.php");
			return;
		}
	}
...
<h2 class="iconAppliS">基盤技術部門に応募する</h2>
<form enctype="multipar/form-data" action="apply_basetechnology_category.php" method="post">
	...
	<table class="application-form" id="input-form">
		...
		<tr>
			...
			<td>
				...
				<select name="set_mailinglist">
					<option value="false"<?php echo (isset($_POST["set_mailinglist"]) && $_POST["set_mailinglist"] == "false" ? " selected" : "") ?>>情報配信を希望しない</option>
					<option value="true"<?php echo (isset($_POST["set_mailinglist"]) && $_POST["set_mailinglist"] == "true" ? " selected" : "") ?>>情報配信を希望する</option>
				</select>
				</div>
			</td>
		</tr>
		...
		<tr>
			<th>基盤技術の名称 *</th>
			<td><input type="text" name="basetechnology-name" value="<?php echo $_POST["basetechnology-name"]; ?>" />
				<?php echo outErrMes("basetechnology-name");?>
			</td>
		</tr>
		<tr>
			<th>基盤技術URL *</th>
			<td><input type="text" name="basetechnology-url" value="<?php echo $_POST["basetechnology-url"]; ?>" />
				<?php echo outErrMes("basetechnology-url");?> 
		...
		<tr>
			...
			<th>基盤技術の概略説明(100字以内で記述して下さい) * [<span id="inputlegth"><?php echo mb_strlen(trim($_POST["application-abstract"]), "UTF-8"); ?>文字</span>]</th>
			<td>
				<textarea name="basetechnology-abstract" onkeyup="ShowLength(value, 'inputlegth');"><?php echo $_POST["basetechnology-abstract"]; ?></textarea>
				<?php echo outErrMes("basetechnology-abstract");?>
			</td>
		</tr>

		<tr>
			<th>基盤技術の詳細説明(作品詳細について記述して下さい)</th>
			<td>
				<textarea name="basetechnology-description"><?php echo $_POST["basetechnology-description"]; ?></textarea>
				<?php echo outErrMes("basetechnology-description");?>
			</td>
		</tr>
