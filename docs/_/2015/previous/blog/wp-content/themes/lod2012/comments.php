...
<?php if ($comments) : ?>
<?php if ( false/*pings_open()*/ ) : ?>
	<a href="<?php trackback_url() ?>" rel="trackback">TrackBack</a> to '<?php the_title(); ?>'.
<?php endif; ?>
...
<li id="comment-<?php comment_ID() ?>" class="<?php echo tj_comment_class() ?>">
	...
	<p><small><?php comment_date('Y年Fj日') ?> at <a href="#comment-<?php comment_ID() ?>"><?php comment_time() ?></a> <? edit_comment_link(__('Edit', 'sandbox'), ' ', ''); ?></small></p>
	</div>
	<div class="clear"></div>
</li>
...
<label for="email"><small>メールアドレス (表示されません) <?php if ($req) _e('(required)'); ?></small></label></p>
