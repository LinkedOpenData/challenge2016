<?php
	...
	if($id > 0){
		$item = $db->executeQuery("select * from visualization_2013 where id = ".$id);
	} else {
		...
		<tr>
			...
			<td>
				...
				<?php 
					...
					$res_dir = opendir( dirname(__FILE__) . '/dat2013/visualization/' );
					...
					closedir( $res_dir );
				?>
					<a href="<?php echo BASE_URL. 'dat2013/visualization/'.$fileName; ?>" target="_blank"><?php $splitted = explode("_", $fileName, 3); echo $splitted[2]; ?></a>
				<?php } ?>
			</td>
		</tr>
...
<form action="modify_visualization_status.php" method="post">
	<h3>登録情報を修正する</h3>
