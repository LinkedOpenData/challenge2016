<?php
	...
	if($id > 0){
		$item = $db->executeQuery("select * from idea_2013 where id = ".$id);
	} else {
...
<div id="contents-form">
	...
	<table id="entrysheet" class="application-form">
		...
		<tr>
			<th>ご氏名</th>
			<td><?php echo $item[0]["name"]; ?><?php echo ($item[0]["is_student"] ? "<i>[学生]</i>" : ""); ?></td>
		</tr>
		<tr>
			<th>ご所属</th>
		...
		<tr>
			...
			<td>
				...
				<?php 
					...
					$res_dir = opendir( dirname(__FILE__) . '/dat2013/idea/' );
					...
					closedir( $res_dir );
				?>
					<a href="<?php echo BASE_URL. 'dat2013/idea/'.$fileName; ?>" target="_blank"><?php $splitted = explode("_", $fileName, 3); echo $splitted[2]; ?></a>
				<?php } ?>
			</td>
		</tr>
