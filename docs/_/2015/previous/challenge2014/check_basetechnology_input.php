<?php
	...
	if($_POST){
		if(isset($_POST['id'])){
			$item = $db->executeQuery("select * from basetechnology_2014 where id = ".$_POST['id']);
		...
		$dateStr = date("YmdHis");
		$changed_icon= 'false';
// echo "アイコンファイルの指定： ".$_FILES['basetechnology-icon']['name'] ." Temp: ". $_FILES['basetechnology-icon']['tmp_name'] ."<br />";
// echo "配列の存在：". is_array($_FILES['basetechnology-icon']) ."要素の存在：".  array_key_exists('name', $_FILES['basetechnology-icon']) ."<br />";
		if ( is_array($_FILES['basetechnology-icon']) && array_key_exists('name', $_FILES['basetechnology-icon']) ){
			if ( ! empty($_FILES['basetechnology-icon']['name']) ) {
// echo "ファイル指定あり：<br />";
			// ファイルが指定された場合
				$tmpIconName = $dateStr."_icon.".get_file_extension($_FILES["basetechnology-icon"]["name"]);
				if (! empty($_FILES["basetechnology-icon"]["tmp_name"]) ) {
					copy($_FILES["basetechnology-icon"]["tmp_name"], dirname(__FILE__) . '/dat/tmp/'.$tmpIconName);
					$changed_icon= 'true';
				}
			} else {
// echo "ファイル指定なし：<br />";
