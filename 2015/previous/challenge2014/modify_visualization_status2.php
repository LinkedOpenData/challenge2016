<?php
	...
	$errorMessages = array();
		if(isset($_POST['modify'])){
			...
			$_POST["visualization-name"] = $item[0]["visualization_name"];
			$_POST["visualization-select"] = $item[0]["type"];
			$_POST["visualization-url"] = $item[0]["url"];
			$_POST["visualization-abstract"] = $item[0]["abstract"];
			$_POST["visualization-description"] = $item[0]["description"];
			$_POST["visualization-used-dataset"] = $item[0]["used_dataset_ids"];
...
<form enctype="multipart/form-data"action="modify_visualization_status2.php" method="post">
	<table class="application-form" id="input-form">
		...
		<tr>
			...
			<th>ビジュアライゼーション作品の概略(100字以内で記述して下さい) *  [<span id="inputlegth"><?php echo mb_strlen($_POST["visualization-abstract"]); ?>文字</span>]</th>
			<td>
		...
		<tr>
			<th>ビジュアライゼーション作品のアイコン (ビジュアライゼーション作品を表すアイコンがあればファイルを指定下さい)</th>
			<td>
				...
				修正する場合のアイコン・ファイルの送信: <input type="file" name="visualization-icon" />
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<?php echo outErrMes("visualization-used-dataset");?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				...
				アイコン・ファイルの送信: <input type="file" name="basetechnology-icon" />
			</td>
		</tr>
		<tr class="info-row">
	...
	<input type="hidden" name="tmpfilname" value="<?php echo $fileName ?>">
