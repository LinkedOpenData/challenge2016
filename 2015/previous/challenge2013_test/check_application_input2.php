<?php
	...
	if($_POST){
		if(isset($_POST['id'])){
			$item = $db->executeQuery("select * from application_2013 where id = ".$_POST['id']);
echo "ID: ".$_POST['id']."password :".$_POST['password']."; hashed pass: ".$item[0]['hashed_password'],";<br />";
echo "Empty(item) :".(empty($item)?'true':'false')." verify :".($pm->verify($_POST['password'], $item[0]['hashed_password'])?'true':'false') ;
